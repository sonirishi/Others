# -*- coding: utf-8 -*-
"""
Created on Sat Dec 24 12:54:38 2016
...
########################################################################################################################################################
## read data
train = pd.read_csv('C:/Users/rsoni106/Documents/Work/Methodology Work/Kaggle/Completed/Allstate/train.csv')
test = pd.read_csv('C:/Users/rsoni106/Documents/Work/Methodology Work/Kaggle/Completed/Allstate/test.csv')
...
for (inTr, inTe) in folds:
    ...
    for j in range(nbags):
        model = nn_model()
        fit = model.fit_generator(generator = batch_generator(xtr, ytr, 1024, True),
                                  nb_epoch = nepochs,
                                  samples_per_epoch = xtr.shape[0],
                                  verbose = 2,validation_data=(xte.todense(),yte),callbacks=[EarlyStopping(patience=2)])
...
fit = model.fit_generator(generator = batch_generator(xtr, ytr, 128, True),
                              nb_epoch = nepochs,
                              samples_per_epoch = xtr.shape[0],
                              verbose = 2,validation_data=(xte.todense(),yte),callbacks=[EarlyStopping(patience=5)])